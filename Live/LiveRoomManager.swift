...
class LiveRoomManager: NSObject {
    ...
    @objc open class func sharedInstancee() -> LiveRoomManager {
    ...
    @objc func isVideoMuted(forUser userId: String) -> Bool {
    ...
    @objc func isAudioMuted(forUser userId: String) -> Bool {
        if let remoteUser = roomUserMap[userId] {
    ...
    @objc func muteRemoteVideo(forUser userId: String, muted: Bool) {
        if let remoteUser = roomUserMap[userId] {
            (remoteUser as! LiveRemoteUser).isVideoMuted = muted
        }
    }
    
    @objc func muteRemoteAudio(forUser userId: String, muted: Bool) {
        if let remoteUser = roomUserMap[userId] {
            (remoteUser as! LiveRemoteUser).isAudioMuted = muted
        }
    }
    
    @objc func onRemoteUserEnterRoom(userId: String) {
        roomUserMap[userId] = LiveRemoteUser.init(userId: userId)
    }
    
    @objc func onRemoteUserLeaveRoom(userId: String) {
     ...
     * 获取视频直播房间列表
     */
    /// weep 想要OC能够调用，需要添加@objc
    @objc func queryLiveRoomList(success: @escaping (_ roomList: [LiveRoomItem])->Void) {
     ...
     * 创建直播房间
     */
    @objc func createLiveRoom(roomId: String?) {
     ...
     * 销毁直播房间
     */
    @objc func destroyLiveRoom(roomId: String?) {
        guard nil != roomId && !(roomId!.isEmpty) else {
            return
        }
